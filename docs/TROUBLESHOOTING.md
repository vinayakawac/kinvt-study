...
4. Auto-popups only inject into **http(s)/file** tabs (`overlay.js`'s guard) or fall back to a popup window. If your active tab was a `chrome://` or extension-store page at the moment the alarm fired, the overlay silently skips that cycle — wait for the next one, or click **Quiz me now** instead, which renders straight into the sidecar itself and works regardless of what tab is active (see [ARCHITECTURE.md](ARCHITECTURE.md#data-flow-for-one-popup)).
...
If you get a small window with its own title bar/minimize/close buttons, that's the `quiz-window.html` fallback. It's supposed to only appear for the *automatic* alarm-triggered popup when injection is blocked (a restricted page like `chrome://`, an extension store, or a debugging/management page — browsers categorically block content-script injection into their own internal pages, for every extension, with no override). **"Quiz me now" from the sidecar should never show this window at all** — it renders directly inside the sidecar's own page instead (see [ARCHITECTURE.md](ARCHITECTURE.md#data-flow-for-one-popup)), so if you see the floating window after clicking "Quiz me now", check `sidepanel.js`'s `startInlineQuiz()` is actually wired to the button's click handler and that `background.js` still has the `BUILD_QUIZ` message case (not the older `START_QUIZ`/`showQuiz()` path).
