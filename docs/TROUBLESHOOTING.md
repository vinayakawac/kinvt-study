...
4. Auto-popups only inject into **http(s)/file** tabs (`overlay.js`'s guard) or fall back to a popup window. If your active tab was a `chrome://` or extension-store page at the moment the alarm fired, the overlay silently skips that cycle — wait for the next one, or click **Quiz me now** instead, which tries the same injection and, only if that's not possible either, renders the card inline in the sidecar itself so you always see *something* (see [ARCHITECTURE.md](ARCHITECTURE.md#data-flow-for-one-popup)).
...
If you get a small window with its own title bar/minimize/close buttons, that's the `quiz-window.html` fallback. It's supposed to only appear for the *automatic* alarm-triggered popup when injection is blocked (a restricted page like `chrome://`, an extension store, or a debugging/management page — browsers categorically block content-script injection into their own internal pages, for every extension, with no override). **"Quiz me now" from the sidecar should never show this window at all**: when a real tab is available, it shows as a translucent overlay on that tab (same as the automatic popup); when no tab is available, it falls back to rendering directly inside the sidecar's own page instead (see [ARCHITECTURE.md](ARCHITECTURE.md#data-flow-for-one-popup)). If you see the floating window after clicking "Quiz me now", check `background.js`'s `BUILD_QUIZ` case is calling `tryInjectOverlay()` rather than `showQuiz()`, and that `sidepanel.js`'s `startInlineQuiz()` checks `res.injected` before ever falling through to `renderInlineQuiz()`.
...
This was a real bug in an earlier version, where "Quiz me now" still went through `background.js#showQuiz()` (the same path the automatic alarm uses) — its "don't stack multiple quizzes" guard could get stuck if a quiz never sent back `QUIZ_CLOSED`/`QUIZ_RESULT`, silently blocking every subsequent manual click for up to 5 minutes with no visible error (see [ERROR_HANDLING.md](ERROR_HANDLING.md#3-quiz-me-now-could-silently-stop-working-after-the-first-click) for that history). "Quiz me now" no longer goes through `showQuiz()` at all — it uses the `BUILD_QUIZ` message, which has no stacking guard to get stuck on, so this specific failure mode isn't reachable anymore. If clicking it repeatedly still does nothing, the more likely cause now is that `chrome.runtime.sendMessage({type:'BUILD_QUIZ'})` isn't resolving — check the service worker console for errors, and confirm `background.js`'s message router still has the `BUILD_QUIZ` case.
